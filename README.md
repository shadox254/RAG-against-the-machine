...
### Usage
Each command below can be run standalone via `uv run python -m student <command>`.
...
### System performance
[à mesurer/reporter si pas encore fait : indexing time, cold start latency, warm retrieval throughput — le sujet impose des seuils précis (≤5 min, ≤60s, ≤90s/1000 questions), donc vaut le coup de les chronométrer et les comparer explicitement]
